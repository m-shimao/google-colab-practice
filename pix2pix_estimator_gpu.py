...
MODEL_DIR = 'gs://{}/model/gpu/{}'.format(bucket_name, time.strftime('%Y-%m-%d-%H-%M-%S'))
# MODEL_DIR = 'gs://{}/model/gpu/{}'.format(bucket_name, time.strftime('%Y-%m-%d'))
...
class Pix2pix:
    ...
    def model_fn(self, features, labels, mode):
        input_img = tf.reshape(features, [-1, 256, 256, 3], name='input_img')
        if mode != tf.estimator.ModeKeys.PREDICT:
            answer_img = tf.reshape(labels, [-1, 256, 256, 3], name='answer_img')
        ...
        return tf.estimator.EstimatorSpec(mode=mode, train_op=tf_group, loss=loss)
...
estimator.train(
    input_fn=lambda:train_input_fn({'batch_size': TRAIN_BATCH_SIZE}),
    max_steps=100
)
...
pred = list(estimator.predict(input_fn=lambda:test_input_fn({'batch_size': TEST_BATCH_SIZE})))

plt.figure(figsize=(15,15))

display_list = [pred[0]['input'], pred[0]['last_unet']]
